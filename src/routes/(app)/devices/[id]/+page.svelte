<script>
    ...
    const device = {
      ...
      lastSeen: '2 min ago',
      ip: '192.168.1.24',
      mac: 'A4:C1:38:2D:91:F0'
    };
  
    const telemetry = [
      { label: 'Temperature', value: '26°C', tone: 'blue' },
      { label: 'Humidity', value: '68%', tone: 'green' },
      { label: 'Battery', value: '92%', tone: 'amber' },
      { label: 'Signal', value: 'Strong', tone: 'purple' }
    ];
  
    const events = [
      { time: '09:42 AM', event: 'Telemetry synced', severity: 'normal' },
      { time: '09:28 AM', event: 'Threshold check passed', severity: 'normal' },
      { time: '08:57 AM', event: 'Battery health reviewed', severity: 'warning' },
      { time: '08:21 AM', event: 'Device reconnected', severity: 'success' }
    ];
  </script>
  ...
  <section class="details-header">
    <div>
      ...
      <h1>{device.name}</h1>
      <p class="subtext">{device.type} • {device.location}</p>
    </div>
  ...
  <section class="hero-grid">
    <article class="hero-card main-card">
      <div class="hero-top">
        ...
        <span class="status-chip online">{device.status}</span>
      </div>
